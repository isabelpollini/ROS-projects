classdef ExampleHelperGazeboSpawnedModel < handle
    %ExampleHelperGazeboSpawnedModel - Class controlling user-spawned models in Gazebo
    %   M = ExampleHelperGazeboSpawnedModel(NAME,WORLD) creates an object that allows for easy
    ...
    %   simulation.
    %
    %   ExampleHelperGazeboSpawnedModel methods:
    ...
    %
    %   ExampleHelperGazeboSpawnedModel properties:
    ...
    methods
        ...
        function obj = ExampleHelperGazeboSpawnedModel(Name, World)
            %ExampleHelperGazeboSpawnedModel Constructor
            ...
            obj.Name = convertStringsToChars(Name);
        ...
        function [position, orientation, velocity] = getState(obj)
            ...
            try
                ...
                if msg.Success
                    ...
                    r = quat2eul(quat);
                    orientation = [r(1)*180/pi r(2)*180/pi r(3)*180/pi];
        ...
        function jointTorque(obj, jointname, duration, effort)
            ...
            serviceMsg.JointName = [obj.Name '::' convertStringsToChars(jointname)];
            
            serviceMsg.Duration.Sec = int32(duration);
            serviceMsg.StartTime.Sec = uint32(0);
            serviceMsg.StartTime.Nsec = uint32(10);
        ...
        function applyForce(obj, linkname, duration, varargin)
            ...
            serviceMsg.BodyName = [obj.Name '::' convertStringsToChars(linkname)];
            
            serviceMsg.Duration.Sec = int32(duration);
            serviceMsg.StartTime.Sec = uint32(0);
            serviceMsg.StartTime.Nsec = uint32(0);
            
            
            wrench_msg = rosmessage('geometry_msgs/Wrench','DataFormat','struct');
        ...
        function setConfig(obj, jointnames, jointpositions)
            ...
            convertStringsToChars(jointnames)
